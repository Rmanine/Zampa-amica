...
class DBAccess {
	...
	*/
	...
	{
		$query = "SELECT p.ID, p.UtenteID, p.AnimaleID, p.Data, p.Note, a.Nome, a.Lingua
				FROM Prenotazione p
	...
	{
		$query = "SELECT p.ID, p.UtenteID, p.AnimaleID, p.Data, p.Note, a.Nome, a.Immagine 
              		FROM Prenotazione p 
              		JOIN Animale a ON p.AnimaleID = a.ID 
              		WHERE p.ID = ?";
		$stmt = mysqli_prepare($this->connection, $query);
		...
		mysqli_stmt_bind_result($stmt, $id, $userID, $animaleID, $data, $note, $nomeAnimale, $immagineAnimale);
	...
	public function addPrenotazione($id_user, $id_animale, $data, $note)
	{
		$query = "INSERT INTO Prenotazione (UtenteID, AnimaleID, Data, Note) VALUES (?, ?, ?, ?)";
		$stmt = mysqli_prepare($this->connection, $query);
		...
		if (!mysqli_stmt_bind_param($stmt, "iiss", $id_user, $id_animale, $data, $note)) {
			mysqli_stmt_close($stmt);
			return false;
		}
	...
	public function updatePrenotazione($id, $data, $note)
	{
		...
		if (!mysqli_stmt_bind_param($stmt, "ssi", $data, $note, $id)) {
			mysqli_stmt_close($stmt);
			return false;
		}
